...
from firstWeb import create_app
from exts import db
from apps.cms import models as cms_models  # 此处加入模型使映射到migrate上
...
def create_role():
    ...
    operator.permissions = CMSPermission.VISITOR | CMSPermission.POSTER | CMSPermission.CMSUSER | \
                           CMSPermission.COMMENTER | CMSPermission.FRONTUSER  # 或关系
...
def add_user_to_role(email, name):  # 给用户添加角色
